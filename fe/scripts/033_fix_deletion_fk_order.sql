-- Migration 033: Fix FK constraint violation in user deletion function
-- Fixes: "update or delete on table "onboarding_messages" violates foreign key constraint"
-- The greeting_message_id FK in authorized_whatsapp_numbers must be cleared before deleting onboarding_messages
...
COMMENT ON FUNCTION public.delete_user_data(UUID, UUID, TEXT) IS
  'Completely deletes all user data from the system. Used for LGPD compliance (self-deletion) and admin user management. Does NOT delete the auth.users record - that must be done separately via Supabase Admin API. v2: Fixed FK constraint order - clears greeting_message_id before deleting onboarding_messages.';
